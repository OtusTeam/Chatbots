...
VOICES_PATH = _LESSON_DIR / "voices.json"
CACHE_DIR = _LESSON_DIR / "cache"
...
def _get_model(language: str = "ru", speaker_id: str = "v5_2_ru", device: str | torch.device | None = None):
    ...
    if _tts_model is None or _tts_lang != language or _tts_speaker_id != speaker_id:
        ...
        _tts_device = device
        # Silero v5 (torch.package) может возвращать None из .to(); перенос in-place
...
def clear_cache() -> int:
    """Удаляет все файлы в CACHE_DIR (папка cache/). Возвращает число удалённых файлов."""
...
if __name__ == "__main__":
    import sys
    args = [a for a in sys.argv[1:] if a != "--clear-cache"]
    if "--clear-cache" in sys.argv[1:]:
        n = clear_cache()
        print(f"Кэш очищен: удалено файлов {n}")
    if not args:
        if "--clear-cache" not in sys.argv[1:]:
            t = "Привет! Это тест синтеза речи."
        else:
            sys.exit(0)
    else:
        t = args[0]
    out = _LESSON_DIR / "cache" / "test_tts.wav"
    out.parent.mkdir(exist_ok=True)
    print("Сохранено:", synthesize(t, output_path=out, device="cuda"))
